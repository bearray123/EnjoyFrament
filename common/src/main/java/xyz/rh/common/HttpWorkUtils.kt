...
class HttpWorkUtils private constructor(){
    ...
    companion object {
        ...
        @JvmStatic
        val instance: HttpWorkUtils by lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
            HttpWorkUtils()
        }
    }
    ...
    /**
     * 这里为什么要把response单独抽出一个泛型，是因为本次demo测试是测dota的接口
     * 而该接口比较奇葩，返回的json不是JsonObject，而是JsonArray。
     * 为了使这个接口后续通用，所以单独用了一个RESP泛型类型来标明接口返回类型，它即可承载T，也可承载ArrayList<T>
     */
    suspend inline fun <reified T : Any, reified RESP: Any> suspendGet(url: String) : RESP {
        return suspendCancellableCoroutine {
            ...
            it.invokeOnCancellation {
                xlog(TAG, "suspendGet:: invokeOnCancellation:: request.cancel")
                call.cancel()
            }
            // 发起GET请求
            call.enqueue(object : Callback{
                ...
                override fun onResponse(call: Call, response: Response) {
                    ...
                    xlog(TAG, "onResponse:: http code = ${response.code()} , resp=${response}")
                    xlog(TAG, "onResponse:: http body === $body ")

                    if (response.isSuccessful) {
                        if (body == null) {
                            it.resumeWithException(RuntimeException("the body is empty..."))
                            return
                        }
                        if (T::class.java == String::class.java) {
                            it.resume(body as RESP)
                        } else {
                            var result : Any = if (body.startsWith("[")) { // 接口返回的是json数组 ：一般服务器接口不会这样设计
                                JSON.parseArray(body, T::class.java)
                            } else { // 接口返回的是json对象
                                JSON.parseObject(body, T::class.java)
                            }
                            it.resume(result as RESP)
                        }
                    } else {
                        xlog(TAG, "onResponse:: http code is not 2xx")
                    }
...
//    suspend inline fun <reified T : Any> suspendGet(url: String) : ArrayList<T> {
//        return suspendCancellableCoroutine {
//            val okHttpClient = OkHttpClient.Builder().build()
//            val call = okHttpClient.newCall(newRequest(url))
//
//            // 取消网络请求
//            it.invokeOnCancellation {
//                log(TAG, "suspendGet:: invokeOnCancellation:: request.cancel")
//                call.cancel()
//            }
//            // 发起GET请求
//            call.enqueue(object : Callback{
//                override fun onFailure(call: Call, e: IOException) {
//                    log(TAG, "onFailure:: e=${e.message}")
//                    it.resumeWithException(e)
//                }
//
//                override fun onResponse(call: Call, response: Response) {
//
//                    val body = response.body?.string()
//                    log(TAG, "onResponse:: http code = ${response.code} , resp=${response}")
//                    log(TAG, "onResponse:: http body === $body ")
//
//                    if (response.isSuccessful) {
//                        if (body == null) {
//                            it.resumeWithException(RuntimeException("the body is empty..."))
...
//                        }
//                        if (T::class.java == String::class.java) {
//                            it.resume(body as ArrayList<T>)
//                        } else {
//                            var result : Any
//
//                            if (body.startsWith("[")) {
//                                result = JSON.parseArray(body, T::class.java)
//                            } else {
//
//                                result = JSON.parseObject(/*body*/herostr, T::class.java)
//                            }
//                            it.resume(result as ArrayList<T>)
//                        }
//                    } else {
//                        log(TAG, "onResponse:: http code is not 2xx")
//                    }
//
//                }
//
...
//    }
    ...
    fun get(url: String) {
        ...
        okHttpClient.newCall(newRequest(url)).enqueue(object : Callback{
            ...
            override fun onResponse(call: Call, response: Response) {
                xlog(TAG, "onResponse:: resp=${response}")
